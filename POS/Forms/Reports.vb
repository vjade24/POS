...
Public Class Reports
    ...
    Private Sub IconButtonPrint_Click(sender As Object, e As EventArgs) Handles IconButtonPrint.Click
        ...
        'End If
    End Sub
    Private Sub Totals(par_period_from, par_period_to)
        Try
            Dim dt As New DataTable()
            Dim conn As SqlConnection = New SqlConnection(connection)
            Dim cmd As SqlCommand = New SqlCommand("sp_transactions_totals", conn)
            cmd.Parameters.Add(New SqlParameter("@par_period_from", par_period_from))
            cmd.Parameters.Add(New SqlParameter("@par_period_to", par_period_to))
